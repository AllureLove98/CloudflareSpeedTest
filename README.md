...
[![Docker Hub](https://img.shields.io/docker/pulls/XIU2/cloudflare-speedtest?style=flat-square&logo=docker)](https://hub.docker.com/r/XIU2/cloudflare-speedtest)
...
# 多个参数
THREADS=300 DOWNLOAD_NUM=20 MAX_DELAY=100 docker-compose up
```

#### 方式三：使用 docker-compose.override.yml（推荐用于多配置场景）
...
# 2. 编辑 docker-compose.override.yml，选择或创建需要的配置

# 3. 启动容器（会自动加载并覆盖默认配置）
docker-compose up
```

**优势**：比 .env 文件更灵活，可以同时覆盖多个 docker-compose 配置。

**文件位置**：[docker-compose.override.yml.example](docker-compose.override.yml.example) - 包含 7 个常见场景的配置模板

#### 方式四：Host 网络模式（性能最优，仅 Linux/macOS）

如果需要最优的网络性能，可以使用 Host 模式：

```bash
# 方式 1: 直接使用 docker-compose.host.yml (Linux/macOS)
docker-compose -f docker-compose.yml -f docker-compose.host.yml up

# 方式 2: 编辑 docker-compose.override.yml，使用示例 7 的配置
cp docker-compose.override.yml.example docker-compose.override.yml
# 取消注释"示例 7"部分
docker-compose up
```

| 网络模式           | 性能       | 隔离性 | 支持平台    | 适用场景               |
| ------------------ | ---------- | ------ | ----------- | ---------------------- |
| **Bridge（默认）** | ⭐⭐⭐     | ⭐⭐⭐ | 全平台      | 日常使用、需要网络隔离 |
| **Host**           | ⭐⭐⭐⭐⭐ | ⭐     | Linux/macOS | 性能测试、高并发       |
...
| TIMES         | 4      | 单个 IP 延迟测速次数         | 1+         |
| DOWNLOAD_NUM  | 10     | 下载测速 IP 数量             | 1+         |
| DOWNLOAD_TIME | 10     | 单个 IP 下载测速最长时间     | 秒数       |
| TEST_PORT     | 443    | 测速端口                     | 1-65535    |
| USE_HTTPING   | false  | 使用 HTTPing 模式而非 TCPing | true/false |

#### 结果过滤条件

| 变量          | 默认值 | 说明         | 范围    |
| ------------- | ------ | ------------ | ------- |
| MAX_DELAY     | 9999   | 延迟上限     | 毫秒    |
| MIN_DELAY     | 0      | 延迟下限     | 毫秒    |
...
| 变量             | 默认值     | 说明                              |
| ---------------- | ---------- | --------------------------------- |
| RESULT_NUM       | 10         | 显示结果数量 (0=不显示结果列表)   |
| OUTPUT_FILE      | result.csv | 输出文件名 (保存到 results/ 目录) |
| DISABLE_DOWNLOAD | false      | 禁用下载测速                      |
| TEST_ALL_IPS     | false      | 测速全部 IP                       |
...
| GIST_ID    | 现有 Gist ID (可选，不填则创建新 Gist)        |

[如何获取 GitHub Token？](https://github.com/settings/tokens)
...
| CF_COLO      | -                       | 指定地区 (IATA 码，英文逗号分隔) |
| DEBUG        | false                   | 调试模式 (显示完整命令)          |

### 常见使用场景

**场景 1: 高性能测速**

需求：更快速地完成测速，获得更多候选 IP
...
# GIST_ID=xxxxxxxxxxxxxxxxxxxxxxxx  (可选，留空创建新 Gist)
...
```bash
# Docker 运行后，结果保存在本地 results/ 目录
cat results/result.csv

# 检查结果数量是否符合 RESULT_NUM 设置
wc -l results/result.csv
```
...
- ✓ `.env` 文件是否存在（需要 `cp .env.example .env`）
- ✓ 参数名是否正确（区分大小写）
- ✓ 运行 `DEBUG=true docker-compose up` 查看实际执行的命令

**问题 2: 权限不足**

```bash
# Windows 需要以管理员身份运行 PowerShell
# Linux/macOS 某些操作可能需要 sudo
...
```bash
# 保留最近 10 个结果文件
ls -t results/result*.csv 2>/dev/null | tail -n +11 | xargs rm -f

# 使用 crontab 定时执行 (每周清理一次)
0 22 * * 0 cd /path/to/CloudflareSpeedTest && \
  ls -t results/result*.csv 2>/dev/null | tail -n +11 | xargs rm -f
```
...
# 参数说明
-n      延迟测速线程数 (1-1000, 默认200)
-t      单个IP延迟测速次数 (默认4)
-dn     下载测速数量 (默认10)
-dt     下载测速时间秒数 (默认10)
-tp     测速端口 (默认443)
-url    测速地址 (默认https://cf.xiu2.xyz/url)
-tl     延迟上限ms (默认9999)
-tll    延迟下限ms (默认0)
-tlr    丢包率上限 (默认1.00)
-sl     速度下限MB/s (默认0)
...
自动上传测速结果到 GitHub Gist。

### 获取 Token

1. GitHub 设置 → Developer settings → Personal access tokens
2. 新建 Token，选择 `gist` 权限
3. 复制 Token 到 `.env` 的 `GIST_TOKEN`
...
**Q: 下载速度显示 0.00?**  
A: 测速地址不可用。使用 `-debug` 模式查看详情，或更换 `-url`。

**Q: 如何在 NAS 上运行?**  
A: 大多数现代 NAS 支持 Docker，通过 NAS 管理界面安装此项目。

**Q: 如何定时运行?**  
A: Linux: `crontab -e` 添加任务  
 Windows: 任务计划程序添加计划任务

**Q: 支持代理吗?**  
A: 本项目不支持代理。Cloudflare 明文禁止代理使用。

## 🔗 相关链接

- [原始项目](https://github.com/XIU2/CloudflareSpeedTest)
- [Docker Hub](https://hub.docker.com/r/XIU2/cloudflare-speedtest)
- [Releases](https://github.com/XIU2/CloudflareSpeedTest/releases)
- [Issues](https://github.com/XIU2/CloudflareSpeedTest/issues)
- [Discussions](https://github.com/XIU2/CloudflareSpeedTest/discussions)
...
**需要帮助?** 查看 [Issues](https://github.com/XIU2/CloudflareSpeedTest/issues) 或 [Discussions](https://github.com/XIU2/CloudflareSpeedTest/discussions)
